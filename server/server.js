...
var url = "mongodb://SGP_Group_5:Shivaraj_123@cluster0-shard-00-00.zwd1e.mongodb.net:27017,cluster0-shard-00-01.zwd1e.mongodb.net:27017,cluster0-shard-00-02.zwd1e.mongodb.net:27017/myFirstDatabase?ssl=true&replicaSet=atlas-zeaq55-shard-0&authSource=admin&retryWrites=true&w=majority";
...
io.on('connection', (socket) => {
    ...
    socket.on('host-join', (data) => {
        ...
        //Check to see if id passed in url corresponds to id of kwizzer game in database
        MongoClient.connect(url, function (err, db) {
            if (err) throw err;
            var dbo = db.db("kwizzerDB");
            var query = { id: parseInt(data.id) };
            dbo.collection('kwizzerGames').find(query).toArray(function (err, result) {
                if (err) throw err;

                //A kwizzer was found with the id passed in url
    ...
    socket.on('host-join-game', (data) => {
        ...
        if (game) {
            ...
            MongoClient.connect(url, function (err, db) {
                ...
                dbo.collection("kwizzerGames").find(query).toArray(function (err, res) {
                    if (err) throw err;
    ...
    socket.on('requestDbNames', function () {
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            var dbo = db.db('kwizzerDB');
            dbo.collection("kwizzerGames").find().toArray(function (err, res) {
                if (err) throw err;
    ...
    socket.on('newQuiz', function (data) {
        MongoClient.connect(url, function (err, db) {
            if (err) throw err;
            var dbo = db.db('kwizzerDB');
            dbo.collection('kwizzerGames').find({}).toArray(function (err, result) {
                if (err) throw err;
                ...
                var game = data;
                dbo.collection("kwizzerGames").insertOne(game, function (err, res) {
                    if (err) throw err;
                    db.close();
                });
                db.close();
